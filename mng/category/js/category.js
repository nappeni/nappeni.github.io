...
function shopCatChange(id, val){
    if (!val || val == '') {
        return false;
    }
    shopCateMain(id, val);
    var index = document.getElementById("category_main"+id).selectedIndex;
    if (index > '0') {
        document.getElementById("chm_subject").value = document.getElementById("category_main"+id).options[index].text;
    }
}
...
function shopCateMain(id, val){
    var tmp_category = parseInt(id);

    if (!val || val == '0') {
        var category_main = tmp_category;
    } else {
        var category_main = tmp_category + 1;
    }
    if (!document.getElementById("mlog"+val)) {
        return false;
    }
    document.getElementById("defalt_category_main").value = id;
    document.getElementById("category_main").value = category_main;
    document.getElementById("mcode").value = val;
    document.getElementById("defalt_code"+category_main).value = val;
    document.getElementById("tmp_mlog").value = "";
    document.getElementById("tmp_mlog").value = document.getElementById("mlog"+val).value;

    // 옵션 삭제
    shopCateMainDelete(category_main);
    // 데이터 불러온다.
    var code = document.getElementById("mcode"+val);
    // 데이터가 없다면.
    if (code == null) {
        // input 추가
        shopCateInput(id, val, '');
    ...
    // 첫번째 분류 -> 선택하세요. 클릭했다면.
    else if (category_main == '1' && val == '0' || val == '' || !val || code == null) {
        return false;
    }
    // 데이터가 존재한다면
    else if (code.value && code.value != '') {
        ...
        for (i=0; i<tmp_str.length; i++) {
            if (tmp_str[i]) {
                ...
                for (k=0; k<str.length; k++) {
                    if (str[k] && k == '0') {
                        // 옵션 추가
                        shopCateMainAdd(category_main, str[1], str[2], str[3], str[4]);
                    }
                } // end for
            }
        } // end for
...
function shopCateInputMain(id, val, all){
    // input 추가
    var id = document.getElementById("inputCodeAdd");
    var objRow = id.insertRow(id.rows.length);
    var objCell = objRow.insertCell(0);

    objCell.innerHTML = "<input type='hidden' id='mcode"+val+"' name='mcode"+val+"' value='"+all+"' />";
}
function shopCateInputMainLog(id, val, all){
    // input 추가
    var id = document.getElementById("inputCodeAddLog");
    var objRow = id.insertRow(id.rows.length);
    var objCell = objRow.insertCell(0);

    objCell.innerHTML = "<input type='hidden' id='mlog"+val+"' name='mlog"+val+"' value='"+all+"' />";
}
...
function shopCateMainAdd(id, val, subject, category_main, code){
    // 옵션 인덱스
    var tmp_mainIndex = document.getElementById("category_main"+id).options.length;
    // 옵션추가
    document.getElementById("category_main"+id).options[tmp_mainIndex] = new Option(subject, val);
}
...
function shopCateMainInsert(){
    // 옵션 아이디
    var id = document.getElementById("category_main").value;
    // 추가할 코드 (category id)
    ...
    // 옵션 인덱스
    var tmpIndex = document.getElementById("category_main"+id).options.length;
    // 데이터
    var name = document.getElementById("mcode"+code).value;
    // 코드 id 값
    var tmp_num = parseInt(document.getElementById("tmp_mcode").value);
    // 코드 id 값에 1 더한다.
    var num = tmp_num + 1;
    // 옵션추가
    document.getElementById("category_main"+id).options[tmpIndex] = new Option(subject, num);
    // 데이터 합산
    document.getElementById("mcode"+code).value = name + "insert:%:"+num+":%:"+subject+":%:"+id+":%:"+code+"|%|";
    // 코드 id 값 변경
    document.getElementById("tmp_mcode").value = num;
    // 데이터 불러온다.
    var tmp_log = document.getElementById("mlog"+num);
    // 데이터가 없다면.
    if (tmp_log == null) {
        // input 추가
        shopCateInputMainLog(id, num, '');
    }
    // 고유키 저장
    document.getElementById("tmp_mlog").value = "";
    var log1 = document.getElementById("mlog"+code).value;
    if (!log1) {
        var log1 = code;
    }
    var log2 = num;
	
    document.getElementById("tmp_mlog").value =  log1+"|"+log2;
    document.getElementById("mlog"+num).value = document.getElementById("tmp_mlog").value;
    // 제목 리셋
    document.getElementById("subject").value = "";
}
...
function shopCateMainChange(){
    // 아이디
    var id = document.getElementById("defalt_category_main").value;
    // 코드
    var code = document.getElementById("mcode").value;
    // 제목
    var subject = document.getElementById("chm_subject").value;
    // 체크
    if (!id || id == '' || !code || code == '') {
        alert("변경할 분류를 선택하세요.");
        return;
    ...
    var defalt_code = document.getElementById("defalt_code"+id).value;
    // 분류
    var category = document.getElementById("category_main"+id).value;
    if (defalt_code == null || id == '1' && category == '0') {
        alert("변경할 분류를 선택하세요.");
        return;
    }
    ...
    if (id && category) {
        // 쪼갠다.
        var tmp_id = document.getElementById("mcode"+defalt_code);
        if (tmp_id == null || tmp_id == '' || !tmp_id) {
            return false;
        }
        var tmp_str = tmp_id.value.split("|%|");
        // 초기화 하고
        document.getElementById("mcode"+defalt_code).value = "";
        // 새로 덮는다.
        for (var i=0; i<tmp_str.length; i++) {
            if (tmp_str[i]) {
                // 쪼갠다.
                ...
                for (k=0; k<str.length; k++) {
                    if (str[k] && k == '0') {
                        // 데이터
                        var name = document.getElementById("mcode"+defalt_code).value;
                        // val 값과 돌린 것이 같으면
                        if (str[1] == code) {
                            document.getElementById("mcode"+defalt_code).value = name + ""+str[0]+":%:"+str[1]+":%:"+subject+":%:"+str[3]+":%:"+str[4]+"|%|";
                        } else {
                            // 데이터 합산
                            document.getElementById("mcode"+defalt_code).value = name + ""+str[0]+":%:"+str[1]+":%:"+str[2]+":%:"+str[3]+":%:"+str[4]+"|%|";
                        }
                    }
                } // end for
            }
    ...
    // 제목 변경
    document.getElementById("category_main"+id).options[index].innerHTML = subject;
}
...
function shopCateMainDelete(id){
    // 삭제될 옵션 마지막 인덱스
    var count = "1";
    if (id == '2') {
        // 옵션 수
        var ChkIndex2 = document.getElementById("category_main2").options.length;
        var ChkIndex3 = document.getElementById("category_main3").options.length;
    
        // 삭제
        for (i = ChkIndex2; i >= count; i--) {
            document.getElementById("category_main2").options[i] = null;
        }
        for (i = ChkIndex3; i >= count; i--) {
            document.getElementById("category_main3").options[i] = null;
        }
    } else if (id == '3') {
        // 옵션 수
        var ChkIndex3 = document.getElementById("category_main3").options.length;
        ...
        // 옵션 수
        var ChkIndex4 = document.getElementById("category_main3").options.length;
        for (i = ChkIndex4; i >= count; i--) {
        }
    } else {
        // 옵션 수
...
function shopCatMainDelete(){
    ...
    // 삭제
    document.getElementById("category_main"+id).options[index] = null;
}
...
function shopCateMainMove(type){
    var category = document.getElementById("defalt_category_main").value;
    var id = document.getElementById("category_main"+category);
    var index = id.selectedIndex;
    if (type == "U") {
        if (index > 1) {
            shopSwapM(id, index, index - 1, type);
        }
    } else if (type == "D") {
        if (index < id.options.length-1) {
            shopSwapM(id, index, index + 1, type);
...
function shopSwapM(id, index, targetIndex, type){
    var onetext = id.options[targetIndex].text;
    var onevalue = id.options[targetIndex].value;
    ...
    swapNameM(val1, val2, index, targetIndex, type);
}
...
function swapNameM(val1, val2, index1, index2, type){
    var id = document.getElementById("defalt_category_main").value;

    if (id == '1') {
        var defalt_code = "0";
        var code = document.getElementById("mcode0").value;
        var tmp_str = code.split("|%|");
    } else {
        var defalt_code = document.getElementById("defalt_code"+id).value;
        var code = document.getElementById("mcode"+defalt_code).value;
        var tmp_str = code.split("|%|");
    }
    // 돌린다.
    for (var i=0; i<tmp_str.length; i++) {
    ...
    swapChangeM(val1, val2, index1, index2, tmp_name1, tmp_name2, type);
}
...
function swapChangeM(val1, val2, index1, index2, tmp_name1, tmp_name2, type){
    var id = document.getElementById("defalt_category_main").value;
    if (id == '1') {
        var defalt_code = "0";
        var code = document.getElementById("mcode0").value;
    ...
    // 새로 셋팅
    document.getElementById("mcode"+defalt_code).value = "";
    document.getElementById("mcode"+defalt_code).value = document.getElementById("tmp_option").value;
}