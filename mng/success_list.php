				...
				<div class="card-body">
							...
							<div class="float-right">
								<form method="get" name="frm_search" id="frm_search" action="<?=$_SERVER['PHP_SELF']?>" class="form-inline" onsubmit="return frm_search_chk(this);">
									<div class="form-group mx-sm-1">
										<input type="button" class="btn btn-primary" value="추가" onclick="location.href='success_form.php'" />
									</div>
                                    <div class="form-group mx-sm-1">
										<input type="button" class="btn btn-primary" value="삭제" onclick="delBanner('./success_update.php')" />
									</div>
								</form>
							</div>
						</div>
					...
					<table class="table table-striped table-hover table-bordered">
						...
						<?
							$_where = " where ";
							$query = "
								select *, a1.idx as bt_idx from banner_t a1 where bt_type = '2'
							";
							$query_count = "
								select count(*) from banner_t a1 where bt_type = '2'
							";
						...
						<tr>
							...
							<td class="text-center">
                                <img class="banner-img" style="border-radius: 0%; width:230px; height: 80px; object-fit:contain;" src="<?= "https://dmonster1705.cafe24.com/data/app_domestic/".$row['bt_file1']?>"/>
							</td>
							<td class="text-center">
								<?=$row['bt_show']?>
							</td>
							...
							<td class="text-center">
								<input type="button" class="btn btn-outline-primary btn-sm" value="수정" onclick="location.href='./success_form.php?act=update&bt_idx=<?=$row['bt_idx']?>&<?=$_get_txt.$_GET['pg']?>'" /> <input type="button" class="btn btn-outline-danger btn-sm" value="삭제" onclick="f_post_del('./success_update.php', '<?=$row['bt_idx']?>');" />
							</td>
						</tr>
						<?
									$counts--;
