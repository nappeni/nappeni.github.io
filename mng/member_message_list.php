				...
				<div class="card-body">
							...
							<form method="get" name="frm_search" id="frm_search" action="<?=$_SERVER['PHP_SELF']?>" class="form-inline" onsubmit="return frm_search_chk(this);">
								<div class="form-group ml-1">
									<select name="sel_search" id="sel_search" class="form-control form-control-sm">
										...
										<option value="a1.mmt_title">제목</option>
										<option value="a1.mmt_content">내용</option>
									</select>
								</div>
								...
								<div class="form-group ml-1">
									<input type="button" class="btn btn-secondary" value="초기화" onclick="location.href='./member_message_list.php'" />
								</div>
								<div class="form-group ml-1">
									<input type="button" class="btn btn-primary" value="신규등록" onclick="location.href='./member_message_form.php'" />
								</div>
							</form>
					...
					<table class="table table-striped table-hover table-bordered">
						...
						<?
							...
							";
							$query_count = "
								select count(*) from member_message_t a1
							";

							if($_GET['search_txt']) {
								if($_GET['sel_search']=="all") {
									$where_query .= $_where."(instr(a1.mmt_title, '".$_GET['search_txt']."') or instr(a1.mmt_content, '".$_GET['search_txt']."'))";
								} else {
						...
						<tr>
							...
							<td>
								<p class="text-truncate" style="max-width: 600px;"><?=$row['mmt_title']?></p>
							</td>
							<td class="text-center">
								<?=$row['mmt_send']?>
							</td>
							...
							<td class="text-center">
								<input type="button" class="btn btn-outline-primary btn-sm" value="수정" onclick="location.href='./member_message_form.php?act=update&mmt_idx=<?=$row['mmt_idx']?>&<?=$_get_txt.$_GET['pg']?>'" /> <input type="button" class="btn btn-outline-danger btn-sm" value="삭제" onclick="f_post_del('./member_message_update.php', '<?=$row['mmt_idx']?>');" />
							</td>
						</tr>
						<?
									$counts--;
