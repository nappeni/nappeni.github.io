...
function popup(url, wval, hval, tval, lval) {
  window.open(
    url,
    'popup',
    'height=' + hval + ',width=' + wval + ',top=' + tval + ',left=' + lval + ',menubar=no,scrollbars=no,status=yes'
  );
}
      ...
      datasets: [
        {
          label: '2019',
          data: [
            42, 42, 30, 30, 18, 22, 16, 21, 22, 22, 22, 20, 24, 20, 18, 22, 30, 34, 32, 33, 33, 24, 32, 34, 30, 34, 19, 34, 18, 10, 22, 24,
        ...
        },
        {
          label: '2018',
          data: [
            35, 28, 32, 42, 44, 46, 42, 50, 48, 30, 35, 48, 42, 40, 54, 58, 56, 55, 59, 58, 57, 60, 58, 54, 38, 40, 42, 44, 42, 43, 42, 38,
      ...
      datasets: [
        {
          label: '2019',
          data: [
            22, 22, 10, 10, 8, 12, 16, 21, 22, 22, 22, 20, 24, 20, 18, 22, 30, 34, 32, 33, 33, 24, 32, 34, 30, 34, 19, 34, 18, 10, 22, 24,
            20, 22, 20, 21, 10, 20, 15, 19, 24,
          ],
          borderColor: ['transparent'],
          borderWidth: 2,
          fill: true,
        ...
        },
        {
          label: '2018',
          data: [
            68, 58, 62, 66, 44, 46, 42, 50, 48, 30, 35, 48, 42, 40, 54, 58, 56, 55, 59, 58, 57, 60, 46, 54, 38, 40, 42, 44, 42, 43, 42, 38,
            43, 41, 43, 50, 58, 58, 68, 72, 72,
          ],
          borderColor: ['transparent'],
          borderWidth: 2,
    ...
    var data = {
      ...
      datasets: [
        ...
        },
        {
          label: '2018',
          data: [
            55, 48, 52, 62, 64, 66, 62, 60, 48, 30, 35, 48, 42, 40, 54, 58, 56, 55, 59, 58, 57, 60, 66, 54, 38, 40, 42, 44, 42, 43, 42, 38,
            43, 41, 33, 30, 28, 28, 32, 36, 42,
          ],
          borderColor: ['transparent'],
          borderWidth: 2,
          fill: true,
...
function f_category_select_reset(ct_rlevel) {
  if (ct_rlevel == 0) {
    $('#category_selectecd1').html(
      '<li class="list-group-flush d-flex justify-content-between align-items-center product_category_list_group"><span class="pl-2 pb-2">중분류</span></li>'
    );
    $('#category_selectecd2').html(
      '<li class="list-group-flush d-flex justify-content-between align-items-center product_category_list_group"><span class="pl-2 pb-2">소분류</span></li>'
    );
    $('#category_selectecd3').html(
      '<li class="list-group-flush d-flex justify-content-between align-items-center product_category_list_group"><span class="pl-2 pb-2">세분류</span></li>'
    );
  } else if (ct_rlevel == 1) {
    $('#category_selectecd2').html(
      '<li class="list-group-flush d-flex justify-content-between align-items-center product_category_list_group"><span class="pl-2 pb-2">소분류</span></li>'
...
function f_pt_supplement_name_num(num) {
  ...
  if (num > 0) {
    for (var i = 1; i <= num; i++) {
      $('#f_pt_supplement_input_box').append(
        '<li class="row align-items-center mt-2"><div class="col-sm-4"><p>추가상품명</p><input type="text" name="pst_title[]" id="pst_title' +
          i +
          '" value="" class="form-control form-control-sm" /></div><div class="col-sm-3"><p>추가상품값</p><input type="text" name="pst_value[]" id="pst_value' +
          i +
          '" value="" class="form-control form-control-sm" /></div><div class="col-sm-3"><p>추가상품가</p><input type="text" name="pst_price[]" id="pst_price' +
          i +
          '" value="" numberOnly class="form-control form-control-sm" /></div></li>'
      );
    }
  }
}
...
function f_pt_option_input_num(num) {
  ...
  if (num > 0) {
    for (var i = 1; i <= num; i++) {
      $('#f_pt_option_input_box').append(
        '<li class="row align-items-center mt-2"><div class="col-sm-4"><p>옵션명' +
          i +
          '</p><input type="text" name="pt_option_name' +
          i +
          '" id="pt_option_name' +
          i +
          '" value="" class="form-control form-control-sm" maxlength="50" placeholder="예)색상" /></div><div class="col-sm-8"><p>옵션값' +
          i +
          '</p><input type="text" name="pt_option_val' +
          i +
          '" id="pt_option_val' +
          i +
          '" value="" class="form-control form-control-sm" maxlength="100" placeholder="예)파랑,노랑,빨강 콤마로 구분해서 입력바랍니다." /></div></li>'
      );
    }
  }
}
...
function f_pt_option_input_direct(num) {
  ...
  if (num > 0) {
    for (var i = 1; i <= num; i++) {
      $('#f_pt_option_list_box').append(
        '<li class="mt-2"><input type="text" name="pt_option_direct_val[]" id="pt_option_direct_val' +
          i +
          '" value="" class="form-control form-control-sm" maxlength="100" placeholder="예)색상" /></li>'
      );
    }
  }
}
...
function f_pat_origin(pat_origin_level, pat_origin2_value) {
  if (pat_origin2_value == '기타') {
    ...
    $.post(
      './product_update.php',
      { act: 'pat_origin', pat_origin_level: pat_origin_level, pat_origin2_value: pat_origin2_value, pat_origin1_value: pat_origin1_value },
      function (data) {
        if (data) {
...
function f_product_supplement_list(update_chk) {
  if (update_chk == '') {
    ...
    var pst_title_t = $('input[name="pst_title[]"]')
      .map(function () {
        return this.value;
      })
      .get();
    var pst_value_t = $('input[name="pst_value[]"]')
      .map(function () {
        return this.value;
      })
      .get();
    var pst_price_t = $('input[name="pst_price[]"]')
      .map(function () {
        return this.value;
      })
      .get();
...
function f_pt_seller_delete(chk_t) {
  if (chk_t == 'Y') {
    $('#f_pt_seller_delete_btn1').addClass('btn-info text-white');
    $('#f_pt_seller_delete_btn2').removeClass('btn-info text-white');
  } else {
    $('#f_pt_seller_delete_btn1').removeClass('btn-info text-white');
    $('#f_pt_seller_delete_btn2').addClass('btn-info text-white');
  }
  $('#pt_seller_delete').val(chk_t);

  return false;
}

function f_sel_ct_id(sel_ct_level, sel_ct_id, obj) {
  $.post('./category_update.php', { act: 'sel_ct_level', sel_ct_level: sel_ct_level, sel_ct_id: sel_ct_id }, function (data) {
    if (data) {
      $('#' + obj).html(data);
    }
  });
...
function f_preview_image_selected(e, obj_id, obj_name) {
  ...
  filesArr.forEach(function (f) {
    ...
    reader.onload = function (e) {
      ...
      $('#' + obj_t + '_box').html(
        '<img style="object-fit:contain; width:250px;" src="' + e.target.result + '" onerror="this.src=\'../images/noimg.png\'" />'
      );
      $('#' + obj_t + '_del').show();
    };
    reader.readAsDataURL(f);
...
function f_return_mem(mt_idx) {
  if (confirm('정말 복구처리하겠습니까?')) {
    $.post('./member_update.php', { act: 'return', mt_idx_t: mt_idx }, function (data) {
      if (data == 'Y') {
        alert('관리자권한 회원 복구 처리되었습니다.');
        document.location.reload();
      }
    });
  }
...
function f_file_delete(obj, file_name, q) {
  $('#' + obj + q + '_help').hide();
  $('#' + obj + q + '_del').val(file_name);
